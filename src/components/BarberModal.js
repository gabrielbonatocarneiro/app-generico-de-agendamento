...
export default ({ show, setShow, user, indexService }) => {
  ...
  useEffect(() => {
    ...
    for (let i = 1; i <= daysInMonth; i++) {
      ...
      let month = d.getMonth() + 1 // os meses começam no 0
      let day = d.getDate()
      ...
      newListDays.push({
        ...
        weekday: daysOfTheWeek[d.getDay()],
        number: i
      })
    }
    ...
    setListHours([])
    setSelectedHour(0)
  }, [user, selectedMonth, selectedYear]) // Monitora essas duas states
      ...
      <ModalArea>
        <ModalBody>
          ...
          {indexService != null &&
            <ModalItem>
              <ServiceInfo>
                <ServiceName>{user.services[indexService].name}</ServiceName>
                <ServicePrice>R$ {user.services[indexService].price.toFixed(2)}</ServicePrice>
              </ServiceInfo>
            </ModalItem>
          }
